declare global {
  interface Window {
    DemographicInfluenceDraw: { start: () => void; cancel: () => void; setRegions: (regions: InfluenceDisplayRegion[]) => void };
  }
}

interface InfluenceDisplayRegion { id: string; name?: string; field?: string; points: [number, number][] }
...
function finish(rawPoints: [number, number][]): void {
  ...
  window.dispatchEvent(new CustomEvent("map:influence-draft", { detail: { id: `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`, points } }));
...
function setRegions(regions: InfluenceDisplayRegion[]): void {
  ...
  for (const region of regions) {
    ...
    group.append("path")
      ...
      .append("title").text(region.name || "Influence region");
  }
}
